...
Elm.Main.make = function (_elm) {
   ...
   var Play = {ctor: "Play"};
   var None = {ctor: "None"};
   var buttontype = $Signal.channel(None);
   var events = $Signal.mergeMany(_L.fromArray([A2($Signal.map,
                                               Tick,
                                               ticks)
                                               ,A2($Signal.map,
                                               Button,
                                               $Signal.subscribe(buttontype))
                                               ,A2($Signal.map,
                                               MouseMove,
                                               $Mouse.position)
                                               ,A2($Signal.map,
   ...
   var playPauseButton = function (sim) {
      return function () {
         var _v1 = sim.state;
         switch (_v1.ctor)
         {case "Paused":
   ...
   $Graphics$Element.right,
   _L.fromArray([makeButton(AddBorderBtn)
                ,makeButton(RemoveBorderBtn)]));
   var curModel = function (sim) {
      return $List.head(sim.models);
   };
   var buttons = function (sim) {
      return A2($Graphics$Element.flow,
      $Graphics$Element.right,
      _L.fromArray([makeButton(Back)
                   ,playPauseButton(sim)
                   ,makeButton(Forward)
                   ,makeButton(Default)
                   ,$Text.asText(A2(roundi,
                   2,
                   curModel(sim).t))]));
   };
   var renderSim = function (sim) {
      return A2($Graphics$Element.flow,
   ...
   var f = F2(function (t,x) {
      return $Basics.sin($Basics.pi * x + 10 * t) * $Utils.exp(0 - Math.pow(x - 0.5,
      2) / 1.0e-3) * $Utils.exp(0 - Math.pow(t,
      2) / 0.1);
   });
...
Elm.Model.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Utils = Elm.Utils.make(_elm);
   var whichLayer = F2(function (str,
   px) {
      return $List.length(A2($List.filter,
      function (x) {
         return _U.cmp(x,px) < 1;
      },
      str.borders));
   });
   var postBoundary = function (str) {
      return str;
   };
   var preBoundary = function (str) {
      return str;
   };
   var getOrZero = F2(function (index,
   arr) {
      return $Maybe.withDefault(0)(A2($Array.get,
      index,
      arr));
   });
   var getX = F2(function (str,i) {
      return A2(getOrZero,i,str.x);
   });
   var getDx = function (str) {
      return A2(getX,
      str,
      1) - A2(getX,str,0);
   };
   ...
   var getUold = F2(function (str,
   i) {
      return A2(getOrZero,
      i,
      str.u_old);
   });
   var getQ = F2(function (str,i) {
      return A2(getOrZero,i,str.q);
   });
   var getNeighbors = F2(function (arr,
   i) {
      return {ctor: "_Tuple3"
             ,_0: A2(getOrZero,i - 1,arr)
             ,_1: A2(getOrZero,i,arr)
             ,_2: A2(getOrZero,i + 1,arr)};
   });
   var finiteDifference = F3(function (str,
   i,
   dt) {
      return function () {
         var $ = A2(getNeighbors,
         ...
         ru = $._2;
         var dx = getDx(str);
         var c = Math.pow(dx / dt,2);
         return -1 * uo + 2 * mu + 0.5 * c * ((mq + rq) * (ru - mu) - (mq + lq) * (mu - lu)) + Math.pow(dt,
         2) * f;
      }();
   });
   ...
   x) {
      return function () {
         var qa = $Array.fromList(layers);
         var lessers = A2($List.filter,
         function (a) {
            return _U.cmp(x,a) < 0;
         },
         borders);
         var l_len = $List.length(lessers);
         var b_len = $List.length(borders);
         return A2(getOrZero,
         b_len - l_len,
         qa);
      }();
   });
   var removeBorderAt = F2(function (str,
   layerID) {
      return function () {
         var newLayers = A3($Utils.spliceList,
         str.layers,
         layerID + 1,
         1);
         var newBorders = A3($Utils.spliceList,
         str.borders,
         layerID,
         1);
         return _U.replace([["borders"
                            ,newBorders]
                           ,["layers",newLayers]
                           ,["q"
                            ,A2($Array.map,
                            A2(calcQ,newBorders,newLayers),
                            str.x)]],
         str);
      }();
   });
   var updateLayerAt = F3(function (str,
   layerID,
   newValue) {
      return function () {
         var newLayers = $Array.toList(A2($Array.set,
         layerID,
         newValue)($Array.fromList(str.layers)));
         return _U.replace([["layers"
                            ,newLayers]
                           ,["q"
                            ,A2($Array.map,
                            A2(calcQ,str.borders,newLayers),
                            str.x)]],
         str);
      }();
   });
   var addBorderAt = F2(function (str,
   x) {
      return function () {
         var newBorders = $List.sort(A2($List._op["::"],
         x,
         str.borders));
         var layerID = A2(whichLayer,
         str,
         x);
         var newLayer = getOrZero(layerID)($Array.fromList(str.layers));
         var $ = A2($Utils.splitList,
         str.layers,
         layerID),
         front = $._0,
         back = $._1;
         var newLayers = $List.append(front)(A2($List._op["::"],
         newLayer,
         back));
         return _U.replace([["borders"
                            ,newBorders]
   ...
   _elm.Model.values = {_op: _op
                       ...
                       ,addBorderAt: addBorderAt
                       ,removeBorderAt: removeBorderAt
                       ,updateLayerAt: updateLayerAt};
   return _elm.Model.values;
};
Elm.Mouse = Elm.Mouse || {};
...
Elm.Utils.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm);
   var splitList = F2(function (lst,
   i) {
      return {ctor: "_Tuple2"
             ,_0: A2($List.take,i,lst)
             ,_1: A2($List.drop,i,lst)};
   });
   var spliceList = F3(function (lst,
   i,
   count) {
      return function () {
         var $ = A2(splitList,lst,i),
         front = $._0,
         back = $._1;
         return $List.append(front)(A2($List.drop,
         count,
         back));
      }();
   });
   var near = F3(function (a,
   b,
   tolerance) {
      return _U.cmp($Basics.abs(a - b),
      tolerance) < 0;
   });
   var linspace = F3(function (start,
   stop,
   n) {
      return function () {
